...
class MyAI(Alg3D):
    ...
    def get_move(
        ...
        # ここにアルゴリズムを書く
        self.black_board, self .white_board = self.board_to_bitboards(board)
        _, move = self.alphabeta(board, depth=1, alpha=-math.inf, beta=math.inf, maximizing=True, player=player)
        return move

    # ビットボード変換ヘルパー
    def board_to_bitboards(self, board: List[List[List[int]]]):
        p1 = 0
        p2 = 0
        for z in range(4):
            for y in range(4):
    ...
    # 有効手列挙
    def valid_moves(self, board: List[List[List[int]]]) -> List[Tuple[int,int]]:
        moves = []
        for x in range(4):
            for y in range(4):
                if board[3][y][x] == 0:
    ...
    def evaluate(self, board: List[List[List[int]]], player: int) -> int:
        ...
        if self.is_win(self.black_board):
            return 1000 if player == 1 else -1000
        if self.is_win(self.white_board):
            return 1000 if player == 2 else -1000
        return 0  # ここを拡張して3連/2連加点可能

    def put_player_bit(self, board: List[List[List[int]]], x: int, y: int, player: int) -> List[List[List[int]]]:
        for z in range(4):
            if board[z][y][x] == 0:
                board[z][y][x] = player
                # ビットボード更新
    ...
    def rollback_player_bit(self, board: List[List[List[int]]], x: int, y: int) -> List[List[List[int]]]:
        for z in range(4):
            if board[3 - z][y][x] != 0:
                board[3 - z][y][x] = 0
                # ビットボード更新
    ...
    # αβ探索
    def alphabeta(self, board: List[List[List[int]]], depth: int, alpha: int, beta: int, maximizing: bool, player: int):
        ...
        if maximizing:
            ...
            for x, y in moves:
                # 置く
                self.put_player_bit(board, x, y, player)
                eval_score, _ = self.alphabeta(board, depth-1, alpha, beta, False, player)
                self.rollback_player_bit(board, x, y)
            ...
            opp = 3 - player
            for x, y in moves:
                self.put_player_bit(board, x, y, opp)
